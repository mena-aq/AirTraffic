...
void initializeFlightPhases() {
    ...
    flightPhases[(int)FlightPhase::HOLDING] = Phase{290, 600, 6000, 14000}; //edited
...
void* simulateFlightArrival(void* arg){  
    ...
    if (!flight->checkFaults()){       
        ...
        //HOLDING->APPROACH
        while(flight->speed>295){
            flight->speed += -4500*(1.0/3600); //v=u+at
            distanceToRunway -= flight->speed/3600.0 + 0.5*-4500*pow((1.0/3600),2); //ut+1/2at^2
