...
public:
    ...
    Flight(int id, FlightType flightType, AirlineType airlineType, AirlineName name) : airlineName(name), id(id), thread_id(-1), type(flightType), airlineType(airlineType), entryTime(0), faulty(false){    
...
public:
    ...
    void acquireRunway(Flight* flight,QueueFlights*& qf){//for now its just for arrival
        ...
        if (currentFlight==nullptr){ //if available, acquire
            pthread_mutex_lock(&lock);
            currentFlight = flight;
            printf("Flight %d acquire runway\n",flight->id);
            return;
        }
        else{
...
public:
    ...
    void dispatchFlight(FlightType flightType){
        ...
        int airline = rand() % NUM_AIRLINE; //randomly generate airline
       ...
       //queue.addFlight(*fl);
        ...
        //if (queue.incomingQueue.empty() && queue.outgoingQueue.empty()) {
          //  std::cout << "No flights to dispatch!" << std::endl;
           // return;
        //}

        //Flight* flight = nullptr;

        // Get the next flight in the incoming queue or outgoing queue
        /*if (!queue.incomingQueue.empty()) {
            flight = new Flight(queue.incomingQueue.front());
            queue.incomingQueue.erase(queue.incomingQueue.begin());
        } else if (!queue.outgoingQueue.empty()) {
            flight = new Flight(queue.outgoingQueue.front());
            queue.outgoingQueue.erase(queue.outgoingQueue.begin());
        }*/
        ...
        if (flight->type == INTERNATIONAL_ARRIVAL || flight->type == DOMESTIC_ARRIVAL) {
            pthread_create(&flightTid[numFlightsDispatched], nullptr, simulateFlightArrival, (void*)arg);            
            pthread_create(&radarTid[numFlightsDispatched],nullptr,radar.flightRadar,(void*)flight);
        } else {
            pthread_create(&flightTid[numFlightsDispatched], nullptr, simulateFlightDeparture, (void*)arg);
            pthread_create(&radarTid[numFlightsDispatched],nullptr,radar.flightRadar,(void*)flight);
        }
