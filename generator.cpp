...
void* receivePayments(void* arg){
    ...
    while (1) {    
        ...
        if (clearanceID!=-1){
            ...
            pthread_mutex_lock(&dashboard.lock);
            dashboard.clearAVNByID(clearanceID);
            pthread_mutex_unlock(&dashboard.lock);
        }
        clearanceID = -1;
...
void* sendAdminChallans(void* arg){
    ...
    while(1){
        //receive airline 
        AirlineName airline;
        int fd = open(PAY_FIFO6,O_RDONLY);
        read(fd,&airline,sizeof(AirlineName)); //hopefully this reads correctly
        close(fd);
        std::cout << "Portal AIrline " << getAirlineName(airline);

        //send all associated challans
        int fd = open(PAY_FIFO1,O_WRONLY); 

        pthread_mutex_lock(&dashboard.lock);
        for (int i=0; i<dashboard.numViolations; i++){
            if (dashboard.AVNs[i]->airline == airline){
                AVN* thisAvn = dashboard.AVNs[i];
                Challan* challan = new Challan(thisAvn->avnID,thisAvn->flightID,static_cast<int>(thisAvn->airline),static_cast<int>(thisAvn->airlineType),thisAvn->amountDue,thisAvn->status, thisAvn->dueDate);
                write(fd,(void*) challan, sizeof(Challan));
            }
        }
        close(fd);
